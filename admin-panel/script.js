...
async function req(path,m='GET',b){if(!(localStorage.getItem(K)||'').trim()) throw new Error('Save API key first in Settings tab.');let r;try{r=await fetch(`${cfg().e}${path}`,{method:m,headers:headers(),body:b?JSON.stringify(b):undefined});}catch(err){throw new Error('Browser blocked direct Appwrite call (CORS/preflight). Use cache bridge for reads or a server-side proxy/worker for CRUD with API keys.');}const t=await r.text();let j;try{j=JSON.parse(t)}catch{j={raw:t}}if(!r.ok){if(r.status===401){const msg=j?.message||'Unauthorized';throw new Error(`${msg}. Key might miss rows.read/rows.write (or old documents scopes).`);}throw new Error(JSON.stringify(j));}return j}
